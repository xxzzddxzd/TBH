...
这种方式不需要知道游戏安装目录。Inject.bat 会自动查找正在运行的 TaskBarHero.exe。

默认速度是 5 倍。注入成功后，游戏窗口上会出现一个小悬浮框:

  TaskBarHero game:1.00.21 plugin:0.4.0.1
...
按住悬浮框空白区域可以拖动位置。
面板里改过的速度、经验倍数、boss/actboss/auto、lock 间隔和悬浮框位置会自动保存到 TaskBarHeroSpeed.ini，下次启动自动读取。
...
让 CrossOver/macOS 显示可拖动的原生标题栏:

  force_game_title_bar=1
...
3. 双击 Inject-CrossOver-macOS.command。
...
如果自动推断 bottle 失败，可以从终端指定:
  ...
  CX_BOTTLE=Steam ./Inject-CrossOver-macOS.command

如果 CrossOver 不在默认位置，可以指定:

  CROSSOVER_WINE="/path/to/CrossOver/.../wine" ./Inject-CrossOver-macOS.command
...
1. Process not found
   先启动 TaskBarHero，再双击 Inject.bat。

2. 注入失败或没有效果
   右键 Inject.bat，选择“以管理员身份运行”。
...
Install-AutoStart.bat 会从正在运行的 TaskBarHero.exe 自动找到游戏目录。
